...
class TestList(unittest.TestCase):
    ...
    def test_list_map(self):
        ...
        self.assertEqual(list_1000.map(lambda x: str(x)).to_list(), [str(x) for x in list_1000])

    def test_list_filter(self):
        self.assertEqual([1, 2, 3].filter(lambda x: x % 2 == 0).to_list(), [2])
        self.assertEqual(list_1000.filter(lambda x: x % 2 == 0).to_list(), [x for x in list_1000 if x % 2 == 0])
    ...
    def test_list_group_by(self):
        self.assertEqual([1, 2, 3, 4, 5].group_by(lambda x: x % 2), {1: [1, 3, 5], 0: [2, 4]})
